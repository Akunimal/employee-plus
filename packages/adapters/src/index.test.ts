...
describe("Ring webhook adapter", () => {
  ...
  it("accepts the exact signed raw payload", () =>
    expect(
      verifyRingWebhookSignature(body, `sha256=${signature}`, "secret"),
    ).toBe(true));
  it("rejects tampering and missing secrets", () => {
    expect(verifyRingWebhookSignature(`${body} `, signature, "secret")).toBe(
      false,
    );
  ...
  it("parses only the narrow event metadata contract", () =>
    expect(parseRingWebhook(body).eventType).toBe("doorbell_press"));
  it("rejects malformed and out-of-contract payloads", () => {
    expect(() => parseRingWebhook("not-json")).toThrowError(EmployeeError);
    expect(() =>
      parseRingWebhook(JSON.stringify({ eventId: "evt-1" })),
