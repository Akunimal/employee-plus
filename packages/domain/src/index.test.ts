...
describe("Employee+ home-service lifecycle", () => {
  ...
  it("lists assets, searches options, checks availability and compares quotes", () => {
    ...
    expect(
      domain.searchServiceOptions({ serviceType: "repair", maxResults: 1 }),
    ).toHaveLength(1);
    expect(
      domain.checkServiceAvailability("option_northstar_standard"),
    ).toHaveLength(3);
    expect(
      domain
        .compareQuotes([
          "option_northstar_priority",
          "option_northstar_standard",
        ])
        .map((quote) => quote.price.amount),
    ).toEqual([129, 189]);
    expect(() => domain.compareQuotes([])).toThrowError(EmployeeError);
    expect(() => domain.compareQuotes(["missing-option"])).toThrowError(
      EmployeeError,
    );
  });
  ...
  it("requires explicit confirmation and preserves the prepared payload", async () => {
    const domain = new EmployeeDomain(createFixtureStore());
    const draft = await domain.prepareBooking(userId, {
      assetId: "asset_water_heater",
      optionId: "option_northstar_standard",
      slotId: "slot_tomorrow_0900",
      addressLabel: "Home",
    });
    expect(draft.requiresExplicitConfirmation).toBe(true);
    expect(draft.payloadHash).toHaveLength(64);
    const booking = await domain.confirmBooking(userId, {
      draftId: draft.draftId,
      confirmationToken: draft.confirmationToken,
      payloadHash: draft.payloadHash,
      idempotencyKey: "book-request-0001",
    });
  ...
  it("rejects a booking for an unavailable asset", async () => {
    const domain = new EmployeeDomain(createFixtureStore());
    await expect(
      domain.prepareBooking(userId, {
        assetId: "asset_missing",
        optionId: "option_northstar_standard",
        slotId: "slot_tomorrow_0900",
        addressLabel: "Home",
      }),
    ).rejects.toThrowError(EmployeeError);
  });
  ...
  it("makes confirmation idempotent", async () => {
    ...
    const input = {
      ...
      confirmationToken: draft.confirmationToken,
      payloadHash: draft.payloadHash,
      idempotencyKey: "book-request-0002",
    };
  ...
  it("rejects a confirmation from another user", async () => {
    ...
    await expect(
      domain.confirmBooking("user-bob", {
        draftId: draft.draftId,
        confirmationToken: draft.confirmationToken,
        payloadHash: draft.payloadHash,
        idempotencyKey: "book-request-0003",
      }),
    ).rejects.toThrowError(EmployeeError);
  });
  ...
  it("changes only the requested appointment slot", async () => {
    const domain = new EmployeeDomain(createFixtureStore());
    const bookingDraft = await domain.prepareBooking(userId, {
      assetId: "asset_water_heater",
      optionId: "option_northstar_standard",
      slotId: "slot_tomorrow_0900",
      addressLabel: "Home",
    ...
    const booking = await domain.confirmBooking(userId, {
      draftId: bookingDraft.draftId,
      confirmationToken: bookingDraft.confirmationToken,
      payloadHash: bookingDraft.payloadHash,
      idempotencyKey: "book-request-0004",
    });
    const changeDraft = await domain.prepareBookingChange(userId, {
      bookingId: booking.bookingId,
      slotId: "slot_tomorrow_1300",
    });
    const changed = await domain.confirmBookingChange(userId, {
      draftId: changeDraft.draftId,
      confirmationToken: changeDraft.confirmationToken,
      payloadHash: changeDraft.payloadHash,
      idempotencyKey: "change-request-0001",
    });
  ...
  it("uses safe temporal language for a correlated Ring event", async () => {
    ...
    const booking = await domain.confirmBooking(userId, {
      ...
      confirmationToken: bookingDraft.confirmationToken,
      payloadHash: bookingDraft.payloadHash,
      idempotencyKey: "book-request-0005",
    });
    const context = await domain.correlateRingEvent(userId, {
    ...
    expect(domain.getRingArrivalContext(userId, "ring-event-1")).toEqual(
      context,
    );
    const unmatched = await domain.correlateRingEvent(userId, {
      eventId: "ring-event-2",
      deviceId: "device-internal",
      eventType: "motion",
  ...
  it("cancels only after explicit confirmation and records an audit event", async () => {
    ...
    const booking = await domain.confirmBooking(userId, {
      ...
      confirmationToken: bookingDraft.confirmationToken,
      payloadHash: bookingDraft.payloadHash,
      idempotencyKey: "book-request-0006",
    });
    const cancelDraft = await domain.prepareBookingCancellation(userId, {
      bookingId: booking.bookingId,
    });
    expect(cancelDraft.requiresExplicitConfirmation).toBe(true);
    const cancelled = await domain.confirmBookingCancellation(userId, {
      draftId: cancelDraft.draftId,
      confirmationToken: cancelDraft.confirmationToken,
      payloadHash: cancelDraft.payloadHash,
    ...
    expect(() =>
      domain.getRingArrivalContext(userId, "missing-ring-event"),
    ).toThrowError(EmployeeError);
    await expect(
      domain.prepareBookingCancellation(userId, {
        bookingId: booking.bookingId,
      }),
    ).rejects.toThrowError(EmployeeError);
  });
  ...
  it("returns status and creates a simulated service document", async () => {
    ...
    const booking = await domain.confirmBooking(userId, {
      ...
      confirmationToken: draft.confirmationToken,
      payloadHash: draft.payloadHash,
      idempotencyKey: "book-request-doc1",
    });
    expect(domain.getServiceStatus(userId, booking.bookingId)).toEqual(booking);
    const document = await domain.createSimulatedDocument(
      userId,
      booking.bookingId,
    );
    expect(document.simulated).toBe(true);
    expect(document.content).toContain("SIMULATED");
    expect(domain.getServiceDocument(userId, document.documentId)).toEqual(
      document,
    );
    expect(() =>
      domain.getServiceDocument(userId, "document_missing"),
    ).toThrowError(EmployeeError);
  });
  ...
  it("hydrates dynamic state from DynamoDB and writes a complete snapshot", async () => {
    const booking = {
      ...
      assetId: "asset_water_heater",
      optionId: "option_northstar_standard",
      providerName: "Northstar Home Care",
      addressLabel: "Home",
      scheduledStart: "2026-09-11T09:00:00.000Z",
      scheduledEnd: "2026-09-11T10:30:00.000Z",
      status: "scheduled",
      version: 1,
    ...
    const stored = [
      {
        pk: "EMPLOYEE_PLUS_STATE",
        sk: "booking#booking_existing",
        entityType: "booking",
        value: booking,
      },
      {
        pk: "EMPLOYEE_PLUS_STATE",
        sk: "document#document_existing",
        entityType: "document",
        value: {
          documentId: "document_existing",
          bookingId: booking.bookingId,
          kind: "invoice",
          label: "Simulated service invoice",
          content: "SIMULATED",
          simulated: true,
          createdAt: "2026-09-10T12:00:00.000Z",
        },
      ...
      {
        pk: "EMPLOYEE_PLUS_STATE",
        sk: "idempotency#user-alice:request-existing",
        entityType: "idempotency",
        value: { key: "user-alice:request-existing", value: booking },
      },
      {
        pk: "EMPLOYEE_PLUS_STATE",
        sk: "ring-event#ring-existing",
        entityType: "ring-event",
        value: {
          eventId: "ring-existing",
          deviceId: "device-internal",
          eventType: "motion",
      ...
      {
        pk: "EMPLOYEE_PLUS_STATE",
        sk: "ring-context#user-alice:ring-existing",
        entityType: "ring-context",
        value: {
          eventId: "ring-existing",
          eventType: "motion",
          occurredAt: "2026-09-11T09:00:00.000Z",
          matchedBookingId: booking.bookingId,
          message: "Activity matches the service window.",
        },
      },
      {
        pk: "EMPLOYEE_PLUS_STATE",
        sk: "audit#audit-existing",
        entityType: "audit",
        value: {
          eventId: "audit-existing",
          userId,
  ...
  it("rejects incomplete DynamoDB configuration", async () => {
    await expect(createDynamoStore({ region: "us-east-2" })).rejects.toThrow(
      "DynamoDB production configuration is incomplete.",
    );
    await expect(
      createDynamoStore({ tableName: "employee-state" }),
    ).rejects.toThrow("DynamoDB production configuration is incomplete.");
  });
});
