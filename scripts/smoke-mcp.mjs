...
const server = spawn(process.execPath, ["apps/mcp-server/dist/index.js"], { stdio: ["ignore", "pipe", "pipe"], env: { ...process.env, NODE_ENV: "test", PORT: "3210" } });
...
try {
  ...
  console.log(JSON.stringify({ status: "ok", protocol: initialized.payload.result.protocolVersion, toolCount: names.length, originValidation: "ok" }));
} finally {
  server.kill("SIGTERM");
